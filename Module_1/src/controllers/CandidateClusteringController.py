"""Candidate clustering controller for grouping similar candidates."""
import numpy as np
...
class CandidateClusteringController:
    ...
    def remove_candidate(self, candidate_id: str):
        removed = False
        for cluster_id, candidates in list(self.model.candidate_metadata.items()):
            if candidate_id in candidates:
                idx = candidates.index(candidate_id)
                candidates.pop(idx)
                self.model.cluster_members[cluster_id].pop(idx)
                removed = True

                if len(candidates) == 0:
                    del self.model.candidate_metadata[cluster_id]
                    del self.model.cluster_members[cluster_id]
                    self.model.centroids = np.delete(self.model.centroids, cluster_id, axis=0)
                    self.model.index.reset()
                    if len(self.model.centroids) > 0:
                        self.model.index.add(self.model.centroids)

                break

        if removed:
            print(f"🗑 Removed candidate {candidate_id}")
        else:
            print(f"⚠ Candidate {candidate_id} not found")

    # -------------------------------------------------
    def remove_candidates(self, candidate_ids: List[str]):
    ...
    def delete_jobs(self, job_ids: List[str]):
        ...
        self.remove_candidates(job_ids)

    # -------------------------------------------------
    def delete_cvs(self, cv_ids: List[str]):
        print(f"🗑 Deleting {len(cv_ids)} CVs")
        self.remove_candidates(cv_ids)

    # -------------------------------------------------
    def save_clusters(self, path: str = "./Module_1/faiss_store/candidate_clusters.pkl"):
        """Save cluster state to disk"""
        self.model.save(path)
        print(f"💾 Saved clusters to {path}")

    def load_clusters(self, path: str = "./Module_1/faiss_store/candidate_clusters.pkl"):
