        ...
        try:
            ...
            for skill in filtered_result["matched_skills"]:
                if skill in skill_to_embedding:
                    ...
                    elif not isinstance(emb, np.ndarray) and emb:
                        # Handle case where embedding is already a list
                        safe_skills.append(skill)
                        safe_embeddings.append(emb if isinstance(emb, list) else list(emb))
