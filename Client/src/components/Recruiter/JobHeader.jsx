...
export const JobHeader = ({ compact = false, job, enriched, lastUpdate }) => {
    ...
    return (
        ...
        <div className="grid grid-cols-2 gap-2 mt-3">
            <button onClick={()=>{navigate(`/edit-job/${job?._id}`)}} className="inline-flex items-center justify-center gap-1.5 py-2 text-white bg-dark-orange rounded-lg hover:scale-105 font-medium text-sm transition">
                <Pencil className="w-3.5 h-3.5" /> Edit Job
            </button>
            <button onClick={() => deleteJob(job?._id)} className="inline-flex items-center justify-center gap-1.5 py-2 text-white bg-light-blue hover:scale-105 rounded-lg  font-medium text-sm transition">
