...
const mapJob = (job) => {
  ...
  const posted = job.created_at
    ? (() => {
        const diff = Math.floor((Date.now() - new Date(job.created_at)) / 86_400_000)
...
const applyFilters = (jobs, { checked = {}, searchValues = {} }) => {
  const activeWorkplaces = ['On site', 'Remote', 'Hybrid'].filter((v) => checked[v])
  ...
  return jobs.filter((job) => {
    ...
    if (activeExp.length > 0 && !activeExp.includes(job.experience)) return false
...
const Skeletons = () => (
  <div className="flex flex-col gap-4">
    {[1, 2, 3].map((n) => (
      <div key={n} className="rounded-3xl border border-gray-100 bg-white px-6 py-5 shadow-md animate-pulse">
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        try {
          const recRes = await fetch(`/api/v1/job/recommended/${CURRENT_USER_ID}`)
          if (recRes.ok) {
            const recData = await recRes.json()
            recList = Array.isArray(recData) ? recData : recData.jobs ?? []
          }
        } catch (err) {
          console.warn('Recommended jobs API failed, falling back to all jobs', err)
        }
        ...
        if (recList.length > 0) {
          ...
          const allList = Array.isArray(allData) ? allData : allData.jobs ?? []
  ...
  const isFiltered =
    ...
    Object.values(filters.searchValues).some((v) => (v ?? '').trim().length > 0)
  ...
  const renderContent = () => {
    ...
    return (
      <div className="flex flex-col gap-4">
        {/* ── No-CV upsell banner ── */}
        {isGenericList && (
          <div className="flex items-center gap-4 rounded-2xl border border-orange/30
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[13px] text-dark-gray3 mt-0.5">
                Upload your CV and we'll show personalised recommendations just for you.
              </p>
            </div>
            <a
              href="/profile"
