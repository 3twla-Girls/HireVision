...
async def get_job(request: Request, job_id: str):
    ...
    try:
        ...
        # This ensures datetime and ObjectId are serialized properly
        job_serialized = jsonable_encoder(job)

        return JSONResponse(content={"job": job_serialized})
...
# Update Job
# =============================
@job_router.patch("/update/{job_id}")
async def update_job(request: Request, job_id: str, job: JobScheme):
    ...
    try:
        ...
        # Serialize for JSON (handles datetime & ObjectId)
        updated_job_serialized = jsonable_encoder(updated_job)

        return JSONResponse(content={"job": updated_job_serialized})

    except Exception as e:
        logger.error(f"Error updating job: {e}")
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": "JOB_UPDATE_FAILED", "error": str(e)}
        )
...
async def delete_job(request: Request, job_id: str):
    ...
    try:
        ...
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={"signal": "JOB_DELETE_FAILED", "error": str(e)}
        )
